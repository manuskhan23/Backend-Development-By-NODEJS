  ...
  save() {
    if (this.id) {
      return db.execute(
        ...
        [this.name, this.price, this.location, this.rating, this.imageUrl, this.description, this.id]
      );
    } else {
      return db.execute(
        "INSERT INTO homes (name, price, location, rating, imageUrl, description) VALUES (?, ?, ?, ?, ?, ?)",
        [this.name, this.price, this.location, this.rating, this.imageUrl, this.description]
      );
    }
  }
